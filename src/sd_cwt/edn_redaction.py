...
class EDNRedactionParser:
    ...
    def parse_edn_with_redaction(self, edn_text: str) -> Tuple[Dict[Any, Any], List[str]]:
        ...
        try:
            ...
            raise ValueError(f"Failed to parse EDN: {e}")
...
class EDNRedactionBuilder:
    ...
    def mark_claim_for_redaction(self, claim_name: Union[str, int], 
                                claim_value: Any, tag: int = 59) -> str:
        ...
        if isinstance(claim_name, str):
            claim_key = f'"{claim_name}"'
        else:
            claim_key = str(claim_name)
        
        return f"{claim_key}: {tag}({value_edn})"
    
    def build_edn_with_redaction(self, claims: Dict[Any, Any], 
                                redaction_config: Dict[Any, int]) -> str:
        ...
        for claim_name, claim_value in claims.items():
            if claim_name in redaction_config:
                ...
                line = "    " + self.mark_claim_for_redaction(
                    claim_name, claim_value, tag
                )
            else:
                # Regular claim without redaction
                if isinstance(claim_name, str):
                    key_edn = f'"{claim_name}"'
                else:
                    key_edn = str(claim_name)
...
def mark_claim_redacted(claim_name: Union[str, int], claim_value: Any, 
                       tag: int = 59) -> str:
