...
class TestCBORExtendedDiagnosticNotation:
    ...
    def test_edn_roundtrip(self):
        ...
        # Convert to CBOR
        cbor_data = edn_utils.diag_to_cbor(original_edn)

        # Convert back to EDN
        result_edn = edn_utils.cbor_to_diag(cbor_data)

        # Parse both to compare (EDN format might differ slightly)
        original_decoded = cbor_utils.decode(edn_utils.diag_to_cbor(original_edn))
        result_decoded = cbor_utils.decode(edn_utils.diag_to_cbor(result_edn))
    ...
    def test_edn_special_values(self):
        ...
        try:
            cbor_data = edn_utils.diag_to_cbor(edn.replace(", 4: undefined", ""))
            decoded = cbor_utils.decode(cbor_data)
            ...
            cbor_data = edn_utils.diag_to_cbor(edn_basic)
            decoded = cbor_utils.decode(cbor_data)
            assert decoded[1] is True
    ...
    def test_edn_comments(self):
        ...
        cbor_data = edn_utils.diag_to_cbor(edn_clean)
        decoded = cbor_utils.decode(cbor_data)
